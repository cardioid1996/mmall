...
public class UserController {
    ...
    @ResponseBody // Jackson直接将返回值转换为json格式
    ...
    @ResponseBody
    public ServerResponse<User> updateInfomation(HttpSession session, User newUser){
        User oldUser = (User)session.getAttribute(Const.CURRENT_USER);
        if (oldUser == null)
            return ServerResponse.createByErrorMessage("用户未登录");
        newUser.setId(oldUser.getId());
        newUser.setUsername(oldUser.getUsername());
        ServerResponse<User> response = iUserService.updateInfomation(newUser);
        if (response.isSuccess()){
